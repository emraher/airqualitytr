...
list_stations <- function() {
  ...
  # # Join with station subtype information
  # StationSubType <- parsed$Object$StationSubType |>
  #   dplyr::rename_with(~c("StationSubType", "StationSubTypeName")) |>
  #   dplyr::distinct()
  #
  # StationIds <- StationIds |>
  #   dplyr::left_join(StationSubType, by = "StationSubType") |>
  #   dplyr::relocate(StationSubTypeName, .after = StationSubType)
  ...
  StationGroupId <- parsed$Object$StationGroupId |>
    ...
    dplyr::distinct() |>
    dplyr::mutate(StationGroup = as.integer(StationGroup))
